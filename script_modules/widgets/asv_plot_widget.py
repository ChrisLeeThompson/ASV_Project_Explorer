...
from script_modules.metadata_query import (
    convert_plot_values, round_plot_values
)
...
class ASVPlotWidget(QWidget):
    ...
    def __init__(
        ...
        decimals: int | None = None, plot_as_text: bool = False,
        ...
        :param plot_as_text: True when this field's values are categorical
            text plotted on a fixed alphabetical y-axis (the field's
            ``PlotAsText`` config setting).
        :param conversion: Optional (scale, offset) map applied to numeric
            values before rounding (the field's ``ConvertToUnit`` config
            setting, pre-resolved against its source unit), or None for
            no conversion.
        ...
        self._plot_as_text = plot_as_text
        ...
        # Category -> integer y-position mapping for text plots (None for
        # numeric/datetime plots). Rebuilt on every plot_data call.
        self._category_positions = None
    ...
    def plot_data(
        ...
        """
        # Convert to the field's target unit, then round to its configured
        # precision (each a no-op when None) up front so the axis line,
        # regression, tooltips, and full-res label all use the same
        # converted, rounded values.
        values = convert_plot_values(values, self._conversion)
        values = round_plot_values(values, self._decimals)
        self._slice_numbers = slice_numbers
        self._values = values
        self._image_names = image_names or []
        # New data invalidates any previous point selection and any
        # previous categorical mapping (reset before the empty-data
        # return so a blanked replot cannot leave a stale mapping).
        self._selected_slice_index = None
        self._category_positions = None
        ...
        # --- Categorical mapping for text plots ---
        # Categories are sorted alphabetically, bottom-to-top, so the axis
        # order is stable regardless of data order or slice range. The
        # integer codes are what gets plotted; self._values keeps the
        # original strings for tooltips, CSV export, and the full-res
        # window value label.
        is_text = self._is_text_data(values)
        if is_text:
            categories = sorted(set(values))
            self._category_positions = {
                cat: pos for pos, cat in enumerate(categories)
            }
            plot_values = [self._category_positions[v] for v in values]
        else:
            plot_values = values

        # --- Plot data points ---
        self.ax.plot(
            slice_numbers, plot_values, "o-",
        ...
        # line when present). The stats box shows average/slope/R² for
        # numeric data or "Avg. Cycle Time" for datetime data. Categorical
        # text plots have no stats box, so the checkbox disables below.
        ...
        if is_text:
            pass
        elif is_datetime and len(values) >= 2:
            ...
            if regression is not None:
                ...
                average_text = self._build_average_text(
                    values, self._decimals
                )
                if average_text:
                    info_text = f"{average_text}\n{info_text}"
        ...
        if is_text:
            # Fixed category ticks with half-step padding so edge
            # categories aren't clipped; a single-category plot keeps a
            # non-degenerate y-range. Runs before the original-limits
            # capture so double-click reset restores the padded view.
            self.ax.set_yticks(range(len(categories)))
            self.ax.set_yticklabels(categories)
            self.ax.set_ylim(-0.5, len(categories) - 0.5)
        self.ax.set_title(
            self._title,
    ...
    @property
    def plot_as_text(self) -> bool:
        """True when this plot's field is configured as categorical text."""
        return self._plot_as_text
    ...
    def highlight_point_by_index(self, idx: int):
        ...
        display_y = self._value_to_axis_y(value)
    ...
    @staticmethod
    def _is_text_data(values: list) -> bool:
        """Check if values are categorical text (a PlotAsText field).

        Extraction guarantees homogeneity: text-flagged fields yield
        all-string values, numeric fields never yield strings.
        """
    ...
    @staticmethod
    def _build_average_text(values: list, decimals: int | None) -> str | None:
        """
        Format the mean of the plotted values.

        Uses the field's configured decimal places when set; otherwise 4
        significant figures (matching the slope).

        :param values: Y-axis values (numeric).
        :param decimals: Per-field decimal places, or None for full
            precision fields.
        :return: Formatted string or None if the mean cannot be computed.
        """
        try:
            average = float(np.mean(np.array(values, dtype=float)))
        except (TypeError, ValueError):
            return None
        if decimals is not None:
            return f"Avg.: {average:.{decimals}f}"
        return f"Avg.: {average:.4g}"
    ...
    def _value_to_axis_y(self, value) -> float:
        """Map a stored data value to its y-axis plot coordinate.

        Datetime values plot at their matplotlib date number; categorical
        text values plot at their category index (the mapping is built by
        plot_data from the same values list, so the lookup cannot miss);
        numeric values are their own coordinate.

        :param value: An entry of ``self._values``.
        :return: The y coordinate the value was plotted at.
        """
        if isinstance(value, datetime):
            return float(mdates.date2num(value))
        if self._category_positions is not None and isinstance(value, str):
            return self._category_positions[value]
        return value
    ...
    def _find_closest_point(
        ...
        for i, (sn, val) in enumerate(zip(self._slice_numbers, self._values)):
            val_num = self._value_to_axis_y(val)
            dx = (x - sn) / x_range
